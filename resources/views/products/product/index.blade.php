...
@section('content')
    ...
    <div class="table">
        <table class="table table-bordered table-striped table-hover">
            <thead>
                <tr>
                    <th>SL.</th><th>Product Name</th><th>Weight</th><th>Length</th><th>Actions</th>
                </tr>
            </thead>                
            <tbody>
            {{-- */$x=0;/* --}}
            @foreach($products as $item)
                ...
                <tr>
                    <td>{{ $x }}</td>
                    <td><a href="{{ url('/product', $item->id) }}">{{ $item->product_name }}</a></td><td>{{ $item->weight }}</td><td>{{ $item->length }}</td>
